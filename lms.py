...
def modify_wb(w,learning_rate,e,p):
	w=w+learning_rate*p*e
	return w
...
def lms_sse(file_name,iteration_no,learning_rate):
	SSEL=[]
	with open(file_name,'r') as calc:
		...
		while MCN<iteration_no:
			...
			for i in l:
				...
				E.append(e)
				if abs(e)!=0.0001:
					w=modify_wb(w,learning_rate,e,p)
			SSE= numpy.matrix(E)
			SSE= SSE*SSE.T
			SSE= SSE.item()
			#SSE=round(SSE,5)
...
def lms_accuracy(file_name,iteration_no,learning_rate):
	SSEL=[]
	with open(file_name,'r') as calc:
		l=calc.read().split('\n')
		MCN=0
